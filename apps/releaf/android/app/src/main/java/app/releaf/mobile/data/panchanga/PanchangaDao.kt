...
interface PanchangaDao {
    ...
    /**
     * All rows that carry a non-empty `special_day`. The repo
     * applies the tokenized search over this in-memory list —
     * a single-`LIKE` SQL query couldn't AND-match tokens across
     * multiple columns (e.g. "Krishna" in `paksha` plus
     * "Janmashtami" in `special_day`) without dynamic SQL, and
     * the dataset is small (~330 such rows) so filtering in
     * Kotlin stays well under a frame budget.
     */
    @Query(
        """
        SELECT * FROM panchanga
