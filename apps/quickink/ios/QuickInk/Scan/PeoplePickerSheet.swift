/*
 * PeoplePickerSheet.swift
 *
 * Attach / detach People to a capture. Invoked from `ScanDetailScreen`.
 * Mirror of Android's `PeoplePickerSheet`.
 */
...
public struct PeoplePickerSheet: View {
    ...
    public var body: some View {
        VStack(alignment: .leading, spacing: AppSpacing.s3) {
            ...
            HStack(spacing: 8) {
                TextField("Add or search a person", text: $model.draft)
            ...
            if model.allPeople.isEmpty {
                Text("No people yet. Type one above to start.")
    ...
    private func row(for person: PersonEntity) -> some View {
        let isSelected = model.selectedIds.contains(person.id)
        return Button(action: { model.toggle(person.id) }) {
    ...
    final class PickerModel: ObservableObject {
        ...
        @Published private(set) var allPeople: [PersonEntity] = []
        ...
        func start() {
            ...
            attachedCancel = repo.observePersonIds(captureId: captureId)
                .receive(on: DispatchQueue.main)
        ...
        func addDraftIfValid() {
            ...
            Task { [userId, captureId] in
                ...
                if let entity = entity {
                    try? await repo.attachPerson(captureId: captureId, personId: entity.id)
        ...
        func commit() async {
            ...
            for id in toAttach {
                try? await repo.attachPerson(captureId: captureId, personId: id)
            }
            for id in toDetach {
                try? await repo.detachPerson(captureId: captureId, personId: id)
            }
        }
    }
}
