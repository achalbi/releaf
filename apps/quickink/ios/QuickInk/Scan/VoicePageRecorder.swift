 ...
 * AVAudioRecorder metering, a progress ring that fills clockwise
 ...
 *
 * Ported from Releaf's `VoicePageRecorder.swift` — same logic, theme
 * tokens swapped for the QuickInk design system.
 *
 * Required Info.plist:
...
public struct VoicePageRecorder: View {
    ...
    /// Recording engine. Either supplied by the caller (so the
    /// parent surface can read `isRecording`/`elapsedMs` and drive
    /// stop/cancel from external chrome) or owned internally when
    /// the recorder is the only consumer.
    ...
    public init(
        ...
        _engine = StateObject(wrappedValue: VoicePageRecorderEngine())
    }

    /// External-engine variant — the parent owns a
    /// [VoicePageRecorderEngine] (typically via its own `@StateObject`)
    /// so it can read state and drive stop/cancel from chrome
    /// outside the recorder body. Internal because the engine type
    /// itself is internal to QuickInk.
    init(
    ...
    private var recordDisc: some View {
        ZStack {
            Circle()
                .stroke(
                    QuickInkColors.accent.opacity(0.45),
    ...
    private var recordingEyebrow: some View {
        HStack(spacing: QuickInkSpacing.s2) {
            Circle()
                .fill(isCancelHover ? VoiceColors.danger : QuickInkColors.accentDeep)
            ...
            Text(isCancelHover ? "RELEASE TO CANCEL" : "RECORDING")
                ...
                .foregroundStyle(isCancelHover ? VoiceColors.danger : QuickInkColors.accentDeep)
        }
    ...
    private var liveWaveform: some View {
        HStack(spacing: 0) {
            ForEach(Array(engine.amplitudes.enumerated()), id: \.offset) { index, level in
                Capsule()
                    .fill(isCancelHover ? VoiceColors.danger : QuickInkColors.accent)
                    .frame(
                        width: 3,
    ...
    private var counter: some View {
        Text(engine.formattedElapsed)
            ...
            .monospacedDigit()
            .foregroundStyle(isCancelHover ? VoiceColors.danger : QuickInkColors.ink)
            .animation(.easeInOut(duration: 0.18), value: isCancelHover)
    }
    ...
    private var cancelHint: some View {
        VStack(spacing: 5) {
            ZStack {
                Circle()
                    .stroke(
                        isCancelHover ? VoiceColors.danger : QuickInkColors.muted,
    ...
    private var stopButton: some View {
        ZStack {
            ...
            Circle()
                ...
                .stroke(
                    isCancelHover ? VoiceColors.danger : QuickInkColors.accent,
            ...
            Circle()
                .fill(isCancelHover ? VoiceColors.dangerSoft : QuickInkColors.accentSoft)
            ...
            if isCancelHover {
                ...
                RoundedRectangle(cornerRadius: 4)
                    .fill(QuickInkColors.textOnAccent)
...
final class VoicePageRecorderEngine: ObservableObject {
    ...
    /// Auto-stop ceiling. Two minutes matches Releaf's recorder.
...
private enum VoiceColors {
    /// Dedicated danger / cancel red. Distinct from the QuickInk
    /// accent palette so "you're discarding this" reads consistently.
