    ...
    private static func parseIso(_ iso: String) -> Parsed? {
        ...
        for fmt in formatters {
            if let date = fmt.date(from: iso) {
                // Read the date components in the user's CURRENT
                // local timezone (`Calendar.current.timeZone`). The
                // spec's "local time of the effective date" reads
                // literally: an 8 pm capture in Tokyo and an 8 pm
                // capture in New York both belong to that location's
                // EVENING bucket, even when the user views from a
                // third timezone. Until v1.1 stores per-capture
                // timezone in the schema, the best approximation is
                // the viewing device's current zone.
                //
