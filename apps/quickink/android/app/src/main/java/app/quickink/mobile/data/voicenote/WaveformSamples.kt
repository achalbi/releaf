 ...
 * per-bucket amplitudes so the voice-note card draws a waveform that
 * reflects the actual audio — not a decorative hash-seeded shape.
 *
 * Decoding is expensive (MediaExtractor + MediaCodec → 16-bit PCM)
 * so results are cached by uri-and-bucket-count in a process-lifetime
 * map. First render of a card triggers the decode on Dispatchers.IO;
 * later renders pull straight from the cache.
 *
 * Output normalized to [0.08, 1.0] — the minimum keeps even silent
 * stretches visually present as tiny bars. Silent recordings render
 * as a row of uniform 0.08 bars so the user can see at a glance that
 * the mic captured nothing.
 *
 * Ported verbatim from Releaf's `WaveformSamples.kt`.
 */
