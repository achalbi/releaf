 ...
 *
 * Port of Releaf Android's `RahuKala.kt` — package rename only.
 */
 ...
 * length varies with latitude. If a per-user location preference is
 * added later, swap these constants for the user's lat/lon.
 */
 ...
 * split, keyed by weekday. Sourced from the standard panchanga rule:
...
/**
 * Display-formatted sunrise + sunset times for [date] at Mysuru in
 * IST. Returns a pair of "HH:mm" strings, or "—" for either field if
 * the solar calculator can't resolve that event (polar latitudes —
 * never expected at Mysuru). Recomputes a single `SunTimes` per call
 * so cell rendering can `remember(date)` the result without paying
 * for two suncalc passes.
 */
...
/**
 * Raw sunrise + sunset [ZonedDateTime] pair for [date] at the given
 * coordinates (defaults to Mysuru, matching the panchanga's anchor).
 * Returns `null` for either event when the solar calculator can't
 * resolve it (polar latitudes only). Exposed publicly so the home
 ...
 *
 * Mirror of iOS `sunTimesFor(_:latitude:longitude:)`.
 */
fun sunTimesFor(
    date: LocalDate,
    latitude: Double = MYSURU_LAT,
    longitude: Double = MYSURU_LON,
    ...
    val times = SunTimes.compute()
        ...
        .execute()
    return times.rise to times.set
}

private const val PLACEHOLDER = "—"

/**
 * Sunrise-anchored Rahu Kala window for [date] at Mysuru. Returns
 * "HH:mm – HH:mm" in IST. Falls back to the published fixed-table
 * when the solar calculator can't resolve a rise/set pair (polar
 * latitudes only — never expected to fire here).
 */
...
/**
 * Published-table approximation, used only when the solar calc can't
 * resolve a sunrise/sunset for the given date and location (polar
 * latitudes; never expected at Mysuru). Times match the conventional
 * table assuming a 6 AM sunrise / 6 PM sunset.
 */
